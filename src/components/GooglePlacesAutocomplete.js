...
export const GooglePlacesAutocomplete = forwardRef((props, ref) => {
  ...
  const _onPress = (rowData) => {
    ...
    if (rowData.isPredefinedPlace !== true && props.fetchDetails === true) {
      ...
      request.open(
        'GET',
        `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(text)}.json?access_token=pk.eyJ1IjoiaGFtaWRyZGV2ZWxvcGVyIiwiYSI6ImNrOHp3eW4ybTBmamEzZnBydmtybmRrZnAifQ.neIu6a_sJjQycKnNxEdxkA`
        // +
          // Qs.stringify({
          //   key: props.query.key,
  ...
  const _renderDescription = (rowData) => {
    ...
    return rowData.place_name || rowData.place_name || rowData.text;
  };
