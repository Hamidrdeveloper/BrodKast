...
export const EventShowScreen = ({navigation}) => {
  const {
    eventListUser,
    eventAddUser,
    eventEditUser,
  ...
  const {groupListUser} = useContext(GroupsContext);
  ...
  useEffect(() => {
    setTimeout(() => {
      if(eventShowData.eventAlerts.length>0){
        var time = 0;


        var h = eventShowData.eventAlerts[0].agoMinute/60
        var s = eventShowData.eventAlerts[0].agoMinute%60;
        if(h>0){
          time =h+' hours ago';
          if((h/24)>=1){
            time =h/24+' days ago';
  ...
  let date = new Date(eventShowData.date);
  let dateStart = new Date(eventShowData.startTime);
          ...
          <ViewMain>
            <ScrollViewCenter>
              ...
              <ViewShow>
                ...
                <LineWidth />
                <ViewCenterItemToRow style={{alignItems:'center',justifyContent:'center'}}>
                  <TitleSmallEventFieldAdd
                    style={{textAlign: 'center'}}>
                    {eventShowData.locationName}
              ...
              <ViewRowButtonSugg>
                ...
                <TouchableOpacity
                  onPress={() => {
                    eventConvertToGroupUser(eventShowData.id);
                  }}>
                  <ButtonSugg
